...
"""
ArkTS Bad-Case Analysis Pipeline (LLM-based) - Simple Random Sampling
--------------------------------------------------------------------
...
#    python arkts_badcase_pipeline_simple.py \
...
def simple_random_sample(rows: List[Dict[str, Any]], n: int, seed: int) -> List[Dict[str, Any]]:
    """
...
def build_prompt(code: str) -> str:
    ...
    if prompt_file.exists():
        try:
            ...
            print(f"Warning: Failed to read prompt.txt: {e}, using default template")
...
def run_round(
    ...
    sampled = simple_random_sample(rows, n=n, seed=seed)
    ...
    # Sort results by item_id and replica to ensure reproducible order
    results.sort(key=lambda x: (x.item_id, x.replica))
...
def main():
    ap = argparse.ArgumentParser(description="ArkTS Bad-Case Analysis Pipeline (LLM-based) - Simple Random Sampling")
